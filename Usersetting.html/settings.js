      ...
      card.innerHTML = `
        ...
        ${canPrintReceipt
          ? `<div class="order-actions">
              <button type="button" class="order-action-btn order-pdf-btn" data-order-id="${encodeToken(order.orderId)}" data-created-at="${encodeToken(order.createdAt)}">Download PDF</button>
              <button type="button" class="order-action-btn order-print-btn" data-order-id="${encodeToken(order.orderId)}" data-created-at="${encodeToken(order.createdAt)}">Generate Receipt</button>
            </div>`
          : ""}
      `;
...
<head>
  ...
  <style>
    *{box-sizing:border-box}
    ...
    .print button{border:1px solid #111;background:#fff;padding:6px 10px;font:inherit;font-size:10px;cursor:pointer}
    @media print{
...
<body>
  ...
  function openReceiptPrint(order) {
  ...
  function bindOrderReceiptPrinting() {
    ...
    orderList.addEventListener("click", async (event) => {
      const button = event.target.closest(".order-pdf-btn, .order-print-btn");
      if (!button) {
        return;
      }
      ...
      if (button.classList.contains("order-pdf-btn")) {
        await downloadReceiptPdf(target, button);
        return;
      }
      openReceiptPrint(target);
    });
  }
